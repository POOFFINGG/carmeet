  ...
  return (
    <Layout>
      <div className="pt-12 px-6 pb-6 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-gradient">Garage</h1>
          <p className="text-muted-foreground font-medium text-sm mt-1">Welcome back, {user?.displayName}</p>
        </div>
      ...
      <div className="px-6 mb-8">
        {user?.role === "viewer" ? (
          <div className="w-full aspect-video rounded-3xl relative overflow-hidden glass-panel flex flex-col items-center justify-center text-center p-6 border-white/20">
            ...
            <h3 className="text-2xl font-bold">Spectator Mode</h3>
            <p className="text-muted-foreground mt-2 text-sm">Find events and apply to view.</p>
          </div>
        ) : carsLoading ? (
          ...
          <div className="w-full aspect-video rounded-3xl glass-panel flex flex-col items-center justify-center p-6 text-center border-dashed border-2 border-white/20">
            ...
            <h3 className="font-bold text-lg mb-1">Add your first car</h3>
            <p className="text-sm text-muted-foreground mb-4">Show off your build to the community</p>
      ...
      <div className="px-6 space-y-6">
        <div className="flex justify-between items-end">
          ...
          <Link href="/profile" className="text-sm text-primary font-bold hover:underline">View All</Link>
        </div>
          ...
          {appsLoading ? (
            ...
            apps.map(app => (
              <div key={app.id} className="glass-panel p-4 rounded-2xl flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-white">{app.eventTitle || "Event"}</h4>
            ...
            <div className="text-center py-8 glass-panel rounded-2xl">
              <p className="text-muted-foreground font-medium">No active applications</p>
              <Link href="/events">
                <Button variant="outline" size="sm" className="mt-4">Find Events</Button>
              </Link>
            </div>
          )}
        </div>
